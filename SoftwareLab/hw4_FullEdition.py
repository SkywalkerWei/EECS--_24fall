...
class MixtureDist:
    def __init__(self, d1, d2, p):
        ...
        self.p = float(p)
    def prob(self, elt):
        return (self.d1.get(elt, 0) * self.p + self.d2.get(elt, 0) * (1 - self.p))
    def support(self):
        return list(set(k for k in self.d1.keys() if self.prob(k) > 0) | set(k for k in self.d2.keys() if self.prob(k) > 0))
    def __str__(self):
        elts = self.support()
        result = 'MixtureDist({'
        result += ', '.join('%s : %s' % (str(x), str(self.prob(x))) for x in elts)
        result += '})'
        return result   
    __repr__ = __str__
'''
...
def squareDist(lo, hi, loLimit=None, hiLimit=None):
    dist1 = {}
    p = 1.0 / (hi - lo)
    loLimit = lo - 1 if loLimit is None else loLimit
    hiLimit = hi + 1 if hiLimit is None else hiLimit
    if loLimit >= hi:
        dist1[loLimit] = 1
    if hiLimit <= lo:
        dist1[hiLimit] = 1
    if loLimit < hi and hiLimit > lo:
        start = max(lo, loLimit)
        end = min(hi, hiLimit)
        if start < end:
            for i in range(start, end):
                incrDictEntry(dist1, i, p)
            if end < hi:
                dist1[end] = (hi - end) * p
    return DDist(dist1)
...
def triangleDist(peak, halfWidth, loLimit=None, hiLimit=None):
    dist1 = {}
    p, lo, hi = 1.0 / (halfWidth)**2, peak - halfWidth + 1, peak + halfWidth
    loLimit = lo - 1 if loLimit is None else loLimit
    hiLimit = hi + 1 if hiLimit is None else hiLimit
    if loLimit >= hi:
        dist1[loLimit] = 1
    ...
    if lo <= loLimit < hi:
        cumulative = sum(dist1.get(j, 0) for j in range(lo, loLimit))
        del [dist1[j] for j in range(lo, loLimit)]
        dist1[loLimit] = cumulative
    if lo < hiLimit <= hi:
        cumulative = sum(dist1.get(k, 0) for k in range(hiLimit, hi))
        del [dist1[k] for k in range(hiLimit, hi)]
        dist1[hiLimit] = cumulative
    return DDist(dist1)
'''
# '''
class MixtureDist:
